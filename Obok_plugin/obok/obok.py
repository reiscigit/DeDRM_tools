...
class KoboLibrary(object):
    ...
    def __getmacaddrs (self):
        ...
        if sys.platform.startswith('win'):
            ...
            try: 
                output = subprocess.Popen('ipconfig /all', shell=True, stdout=subprocess.PIPE, text=True, encoding=os.device_encoding(0)).stdout
                for line in output:
                    m = c.search(line)
                    if m:
                        macaddrs.append(re.sub("-", ":", m.group(1)).upper())
            except:
                output = subprocess.Popen('wmic nic get MACAddress', shell=True, stdout=subprocess.PIPE, text=True).stdout
                for line in output:
                    m = c.search(line)
                    if m:
                        macaddrs.append(re.sub("-", ":", m.group(1)).upper())
